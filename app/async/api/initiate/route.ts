...
export async function POST(request: NextRequest) {
  try {
    ...
    const { role, title, value } = body

    if (!role || value === undefined)
      return NextResponse.json({ error: 'Missing required fields: role, value' }, { status: 400 })
    ...
    const encryptedValue = [iv, encryptedBuffer].map((b) => b.toString('base64url')).join('.')
